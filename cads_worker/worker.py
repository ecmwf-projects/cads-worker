...
def enter_task_temp_dir() -> Iterator[str]:
    old_cwd = os.getcwd()

    worker = get_worker()
    key = worker.get_current_task()
    task_path = cads_broker.utils.rm_task_path(worker, key)
    ...
    try:
        ...
        os.chdir(old_cwd)
        cads_broker.utils.rm_task_path(worker, key)
...
def submit_workflow(
    ...
    try:
        sorted_request = {k: request[k] for k in sorted(request.keys())}
        with enter_task_temp_dir():
            result = cacholote.cacheable(adaptor.retrieve)(request=sorted_request)
    ...
    with context.session_maker() as session:
        cads_broker.database.set_request_cache_id(
            request_uid=job_id,
            cache_id=result.id,
            session=session,
        )
